...
public final class BPJSCekRujukanKartuPCare extends javax.swing.JDialog {
    ...
    private String kdkel="",kdkec="",kdkab="",kdprop="",nosisrute="",BASENOREG="",ADDANTRIANAPIMOBILEJKN="no",URUTNOREG="",link="",statuspasien="",pengurutan="",tahun="",bulan="",posisitahun="",awalantahun="",awalanbulan="",
            ...
            umur="",umurdaftar="0",namakeluarga="",no_peserta="",kelurahan="",kecamatan="",sttsumur="", norefAntrol="",
            kabupaten="",pekerjaanpj="",alamatpj="",kelurahanpj="",kecamatanpj="",datajam="",jamselesai="",
            kabupatenpj="",hariawal="",requestJson,URL="",nosep="",user="",prb="",peserta="",respon="200", jnsKunAntrol="",
        ...
        if(JenisPelayanan.getSelectedIndex()==1){
            ...
            if(Sequel.menyimpantf2("reg_periksa","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",19,
                    ...
                    if(ADDANTRIANAPIMOBILEJKN.equals("yes")){
                        ...
                        antrol.setData(TNoRw.getText(), TNo.getText(), Sequel.cariIsi("select nm_pasien from pasien where no_rkm_medis='" + TNo.getText() + "'"),
                                NoKartu.getText(), KdPoli.getText(), NmPoli.getText(), KdDPJPLayanan.getText(), NmDPJPLayanan.getText(),
...
//            }
    ...
    private void simpanAntrol() {
        try {
            if (!akses.getNomorAntrian().equals("")) {
                Sequel.menyimpanignore("antrol_bpjs", "'" + TNoRw.getText() + "','','" + Valid.SetTgl(TanggalSEP.getSelectedItem() + "") + "',"
                        + "'" + Sequel.cariIsi("select time(waktu_cetak) from antrian_nomor where no_antrian='" + akses.getNomorAntrian() + "' and date(waktu_cetak)=date(now())") + "',"
                        + "'" + Sequel.cariIsi("select time(waktu_panggil) from antrian_pemanggil where no_antrian='" + akses.getNomorAntrian() + "' and date(waktu_panggil)=date(now())") + "',"
                        + "'" + TanggalSEP.getSelectedItem().toString().substring(11,19) + "',"
                        + "'','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','-'");

                Sequel.menyimpanignore("antrian_history", "'" + akses.getNomorAntrian() + "',"
